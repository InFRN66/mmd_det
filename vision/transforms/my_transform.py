...
def scale_transform(img, boxes, scale=0.95, x_shift=0, y_shift=0, mode=False):
    """
    Args : 
        img : image / [h, w, c] , 
    ...
    if mode:
        ...
        if scale >= 1.0: # shift + 拡大
            seq = iaa.Sequential([
                iaa.Affine(translate_px={'x':x_shift, 'y':y_shift}),
...
class ScaleTrans:
    ...
    def __call__(self, org_img, org_boxes, DIFFXs, DIFFYs, mode=False):
        img_list = list()
        boxes_list = list()
        for i in range(self.num_augment):
            img, boxes = self.transform(org_img, org_boxes, scale=self.scale_coeff**i, x_shift=int(DIFFXs[i]), y_shift=int(DIFFYs[i]), mode=mode)
...
class GridTrans:
    ...
    def __call__(self, images, boxes, labels, x_shift, y_shift, mode):
        ...
        if mode:
            ...
            aug = iaa.Affine(translate_px={'x':x_shift, 'y':y_shift})
    ...
    def augment(self, images, boxes, labels, direction, num=10, mode=False):
        '''
        ...
        for i in range(num):
            img, b, lab = self(images, boxes, labels, x_shift=i*direction[0]*self.shift_range, y_shift=i*direction[1]*self.shift_range, mode=mode)
            IMG.append(img)
            BOX.append(b)
            LABEL.append(lab)
...
class AnchorTrans:
    ...
    def __call__(self, images, boxes, labels, x_scale, y_scale, mode):
        assert images.ndim == 3 and images.shape[-1] == 3
        assert images.min() >= 0 and images.max() <= 255
        assert boxes.ndim == 2 and boxes.shape[0] == 1 # [n_box, 4], 1batch
        bb = ia.BoundingBoxesOnImage([
        ...
        if mode:
            ...
            aug = iaa.Affine(scale={'x':x_scale, 'y':y_scale})
    ...
    def augment(self, images, boxes, labels, num=10, mode=False):
        '''
        一枚の画像を複数シフト画像にaugment
        Args: 
            images: 入力画像，[h, w, c]
        ...
        IMG, BOX, LABEL = [ list() for _ in range(3)]
        for i in range(num):
            img, b, lab = self(images, boxes, labels, x_scale=(self.x_scale)**i, y_scale=(self.y_scale)**i, mode=mode)
            IMG.append(img)
            BOX.append(b)
            LABEL.append(lab)
