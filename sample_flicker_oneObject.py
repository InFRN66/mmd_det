...
class Frame:
    ...
    def compare_IoU_one(self, current_scale, track_Id, idx, dets, true_box, IoU_thresh=0.5):
        ...
        print(f'dets: {dets}')
        print(f'true: {true_box}')
        print(f'IoU: {IoU_store}')
        ...
        # IoUを満たすboxがある:
            ...
            print('正解に最も近いboxは{}, {}/{}番目.'.format(IoU_v[0], indicies[0]+1, len(IoU_store)))
            ...
            print('compare :', self.track_box[current_scale, track_Id, idx])
    ...
    def analyse(self, prob_diff, frame_status, track_Id, prob_thresh=0.4):
        '''
    ...
    def analyse_probabiity_percent(self, track_Id, frame_status, obj):
        '''
        前フレームとのprobの差分を調べる / 前フレームのprobを１として後フレームがどれだけ下がったか
        '''
        print()
        probability = self.track_box[:, track_Id, obj, 0].cpu().numpy()
        print(f'probability in this object:\n {np.array(probability)}') # shape = 総フレーム数
        print()
        F = np.array([1,-1]) # フィルタ
        ...
        # 前1フレームで出力が大きい（0.5以上）：　以外の場合はfragmentとして検出しない
        # for j in range(1, len(new)):
            # if np.array(probability)[j-1] < 0.7:
            # if np.array(probability)[j-1] < 0.5:
                # new[j] = 0 # 前フレームとの差分を0とする(検出されないようにする)

        # 前後１フレーム両方で出力が大きい：　以外の場合はfragmentとして出力しない
        for j in range(1, len(new)-1):
            if j == len(new)-2:
                new[-1] = 0 # 最後のフレームは検出されないように
                continue
            else:
                if np.array(probability)[j-1] < 0.5 or np.array(probability)[j+1] < 0.5: # or なのは対偶取ってるから
                    new[j] = 0 # 前フレームとの差分を0とする(検出されないようにする)

        # for j in range(2, len(new)-1):
        #     if np.array(probability)[j-1] < 0.5 or np.array(probability)[j+1] < 0.5:
        #         new[j] = 0 # 前フレームとの差分を0とする(検出されないようにする)
